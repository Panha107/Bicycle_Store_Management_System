...
class Customer{
	...
	public:
		...
		void UpdateResister()	
		{
			...
			drawBoxSingleLine(50,22,25,1,11);
			foreColor(5);
			gotoxy(53,8);	cin.getline(name,15);		fflush(stdin);//	cin.clear();
			gotoxy(53,11);	cin.getline(username,20);	fflush(stdin);
			gotoxy(53,14);	hidePassword(password);		fflush(stdin);
			gotoxy(53,17);	cin.getline(gender,10);		fflush(stdin);
			gotoxy(53,20);	cin.getline(address,20);	fflush(stdin);
	...
	public:
		...
		{
			foreColor(9);
			cout <<"\n      "<<left<<setw(6)<<(idcus+=1)<<" "<<setw(15)<<name<<"   "<<setw(16)<<username<<"   "<<setw(10)<<password<<"   "<<setw(10)<<gender<<" "<<setw(20)<<address<<" "<<setw(18)<<telegram;
		}
...
{
	bool isFound = false;
	cout <<"Input Username: "; cin>>ctm.rusername;
	cout <<"Input Password: "; cin>>ctm.rpassword;
